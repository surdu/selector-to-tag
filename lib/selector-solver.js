...
class SelectorSolver {
		...
		if ((solveTagsOnly && tag) || classes.length > 0 || id) {
			...
			if (isSelfClosing && closeSelfclosingTags) {
				...
				if (expandBlockTags) {
					...
					snippet = snippet.insertAt(snippet.indexOf("</"), "\n$2\n");
				}
				else {
	...
	handleCommand(event) {
		...
		const closeSelfclosingTags = atom.config.get('selector-to-tag.closeSelfclosingTags');
		const blockTags = atom.config.get('selector-to-tag.blockTags');

		const selector = match[1];
		const solvedElement = this.solveSelector(selector, solveTagsOnly, closeSelfclosingTags, expandBlockTags, blockTags);
		...
		if (expandBlockTags && solvedElement.isBlock) {
			...
			if (this.availableAPI.snippetsAPI) {
				...
				console.log("solvedElement.snippet:", solvedElement.snippet);
