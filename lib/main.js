const SelectorSolver = require("./selector-solver");
...
module.exports = {
	config: {
		solveTagsOnly: {
			...
			description:
				"Solve to tag even if the selector is only the tag with no id and/or class.",
		},
		...
		activeForGrammar: {
			...
			description:
				"Active only if HTML grammar is selected. The `File Extensions` option will be ignored if this is enabled.",
		},
	...
	activate() {
		new SelectorSolver(this.availableAPI);
	},

	consumeSnippets(snippetsAPI) {
		this.availableAPI.snippetsAPI = snippetsAPI;
	},
};
